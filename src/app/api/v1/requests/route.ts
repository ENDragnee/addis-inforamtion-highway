...
async function handler(request: AuthenticatedRequest) {
  ...
  const { institution, signature } = request;
  const requesterId = institution.id; // The authenticated institution's ID
  ...
  try {
    ...
    const { providerId, dataOwnerId, dataSchemaId, relationshipId, expiresAt } = validation.data;
    ...
    await prisma.dataRequestSignature.create({
      data: {
        ...
        signature: signature, // Use the validated signature
      },
    });
